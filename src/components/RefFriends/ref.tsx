...
const Refer: React.FC<ReferProps> = ({
  ...
  const inviteLink = `https://t.me/Bitbrawlofficial_bot    (Reffer Code:${userId})`;
  const sharelink = `https://t.me/share/url?url=https://t.me/Bitbrawlofficial_bot (Reffer Code:${userId})`;
  ...
  const handleReferralSubmission = async (event: React.FormEvent) => {
    ...
    if (referrerId) {
      ...
      try {
        ...
        balanceRef.current.value += 20000; // Add 20000 points to the balance
        ...
        if (error instanceof Error) {
          setErrorMessage(
            `Failed to handle new user with referral: ${error.message}`
          );
        } else {
          setErrorMessage(
            `Failed to handle new user with referral: ${String(error)}`
          );
        }
      }
    } else {
